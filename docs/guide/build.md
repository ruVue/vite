...
Обратите внимание, что по умолчанию Vite обрабатывает только синтаксические преобразования и **не охватывает полифилы**. Вы можете проверить [Polyfill.io](https://polyfill.io/v3/), который представляет собой сервис, который автоматически генерирует пакеты полифилов на основе строки UserAgent браузера пользователя.
...
Вы можете настроить способ разделения фрагментов с помощью `build.rollupOptions.output.manualChunks` (смотрите [документацию Rollup](https://rollupjs.org/configuration-options/#output-manualchunks)). До версии Vite 2.8 стратегия разбиения по умолчанию делила фрагменты на `index` и `vendor`. Это хорошая стратегия для некоторых SPA, но сложно предложить общее решение для каждого целевого варианта использования Vite. Начиная с Vite 2.9, `manualChunks` больше не модифицируется по умолчанию. Вы можете продолжать использовать стратегию Split Vendor Chunk, добавив `splitVendorChunkPlugin` в свой файл конфигурации:
...
::: warning
При использовании этого плагина вам следует использовать функциональную форму `build.rollupOptions.output.manualChunks`. Если используется форма объекта, плагин не будет иметь никакого эффекта.
:::
...
В режиме библиотеки все использование `import.meta.env.*` статически заменяется при сборке для производства. Однако использование `process.env.*` не допускается, поэтому потребители вашей библиотеки могут его динамически изменять. Если это нежелательно, вы можете использовать `define: { 'process.env.NODE_ENV': '"production"' }`, например, для их статической замены
:::
